...
static void mouse_read(lv_indev_t * indev_drv, lv_indev_data_t * data)
{
  ...
  INTN MoveMinX = (int)mSimplePointer->Mode->ResolutionX * 20;
  INTN MoveMinY = (int)mSimplePointer->Mode->ResolutionY * 20;

  //Print(L"%X %X %X %X\n",MoveMin,hor_res,ver_res,SimpleState.RelativeMovementX);

  if (!EFI_ERROR (Status)) {
    data->point.x += (SimpleState.RelativeMovementX * hor_res) / MoveMinX;
    data->point.y += (SimpleState.RelativeMovementY * ver_res) / MoveMinY;
    data->state = (SimpleState.LeftButton & BIT0) ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED; // left button
  } else {
    data->state = LV_INDEV_STATE_RELEASED;
  }
